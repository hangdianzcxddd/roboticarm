...
Run on Windows from ``piper_project``:
    ...
    python test/test_click_send_tcp.py --host <linux-vm-ip> --port 5005

Left-click the aligned RGB image to send the camera XYZ point to the Linux-side
``tcp_motion_server.py``. Press ``Esc`` to exit.
"""
...
from communication.protocol import CameraPointCommand, send_camera_point
